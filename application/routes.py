...
def show():
  ...
  for x in res:
    ...
    p1p=Powers.query.filter(Powers.power==x.p1.data.upper()).first()
    p2p=Powers.query.filter(Powers.power==x.p2.data.upper()).first()
    p3p=Powers.query.filter(Powers.power==x.p3.data.upper()).first()
