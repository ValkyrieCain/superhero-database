...
def publisher():
  if hero.validate_on_submit():
    results=Superheroes.query.filter(superhero.publisher==search.publisher.data).all()
    return render_template("show.html", superherodata=results)
  return render_template("searchpublisher.html", search=hero)
@app.route('/search/name', methods=['GET','POST'])
def name():
  if hero.validate_on_submit():
    results=Superheroes.query.filter(superhero.name==search.name.data).all()
    return render_template("show.html", superherodata=results)
  return render_template("searchname.html", search=hero)
@app.route('/search/alterego', methods=['GET','POST'])
def alterego():
  if hero.validate_on_submit():
    results=Superheroes.query.filter(superhero.alterego==search.alterego.data).all()
    return render_template("show.html", superherodata=results)
  return render_template("searchalterego.html", search=hero)
@app.route('/search/power', methods=['GET','POST'])
def power():
  if hero.validate_on_submit():
    results=Superheroes.query.filter(superhero.p1==search.power.data or superhero.p2==search.power.data or superhero.p3==search.power.data).all()
    return render_template("show.html", superherodata=results)
  return render_template("searchpower.html", search=hero)
@app.route('/search/team', methods=['GET','POST'])
def team():
  if hero.validate_on_submit():
    results=Superheroes.query.filter(superhero.team==search.team.data).all()
    return render_template("show.html", superherodata=results)
  return render_template("searchteam.html", search=hero)
@app.route('/search/sidekick', methods=['GET','POST'])
def sidekick():
  if hero.validate_on_submit():
    results=Superheroes.query.filter(superhero.sidekick==search.sidekick.data).all()
    return render_template("show.html", superherodata=results)
  return render_template("searchsidekick.html", search=hero)
@app.route('/search/nemesis', methods=['GET','POST'])
def nemesis():
  if hero.validate_on_submit():
    results=Superheroes.query.filter(superhero.nemesis==search.nemesis.data).all()
    return render_template("show.html", superherodata=results)
